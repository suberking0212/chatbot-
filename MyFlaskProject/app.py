...
BAIDU_API_KEY = 'eIfbnwnnIRlQDaHMhXZULSfv'
BAIDU_SECRET_KEY = 'FurHAiOAn9Dp8dx0rMbOf3xaTbTeHS2t'
...
def generate_gpt_reply(user_input):
    ...
    data = {
        ...
        "messages": [
            {"role": "system", "content": "你是一个帮助解决问题的助手。"},
...
def index():
    if request.method == 'POST':
        ...
        if text_input:
            # 处理文本输入
            gpt_reply = generate_gpt_reply(text_input)
            audio_data = text_to_speech(gpt_reply, lang=language)
            ...
            # 使用 GPT 回复
            gpt_reply = generate_gpt_reply(speech_text)
            audio_data = text_to_speech(gpt_reply, lang=language)
            return render_template('index.html', text_reply=gpt_reply, audio_reply=audio_data)
